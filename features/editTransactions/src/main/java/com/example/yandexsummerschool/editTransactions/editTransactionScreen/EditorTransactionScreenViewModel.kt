...
class EditorTransactionScreenViewModel @Inject constructor(
    ...
    private val _articles = MutableStateFlow<List<com.example.yandexsummerschool.domain.models.ArticleModel>>(emptyList())
    ...
    fun changeCategory(article: com.example.yandexsummerschool.domain.models.ArticleModel) {
        val currentState = state.value
        if (currentState is EditorTransactionScreenState.Content) {
            val updatedTransaction =
                currentState.transaction.copy(
