...
class MyHistoryScreenViewModel @Inject constructor(
	...
	private val _startOfPeriod = MutableStateFlow(millsToDate(0L))
		...
		viewModelScope.launch {
			...
			when (val result = getExpensesUseCase(
				...
				is Result.Success -> {
					...
					if (expenses.isEmpty()) _myHistoryScreenState.value = HistoryScreenState.Empty
					else {
						val history = expenses.map { it.toHistoryItem() }
						val content = HistoryScreenState.Content(
							history = history,
